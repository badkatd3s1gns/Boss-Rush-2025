...
func _physics_process(delta):
	...
	if Input.is_action_pressed("sprint") and player.velocity.length() and player.is_on_floor():
	...
	if !Input.is_action_pressed("action_aiming"): $Pivot/SpringArm/Camera3D.fov = lerpf($Pivot/SpringArm/Camera3D.fov, 83.5 if (Input.is_action_pressed("sprint") and player.velocity.length() and player.is_on_floor()) else 75, delta*7.0)
